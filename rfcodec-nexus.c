...
void nexus_decode(uint8_t protocol, uint64_t raw, uint8_t repeat) {
	...
	if (rfcfg->sysfslike) {
		...
		create_sysfslike(rfcfg->sysfslike, "temp", ctemp, "%s%d%d", "NEXUS", i, c);
		create_sysfslike(rfcfg->sysfslike, "humi", chumi, "%s%d%d", "NEXUS", i, c);
		create_sysfslike(rfcfg->sysfslike, "batt", cbatt, "%s%d%d", "NEXUS", i, c);
	}
